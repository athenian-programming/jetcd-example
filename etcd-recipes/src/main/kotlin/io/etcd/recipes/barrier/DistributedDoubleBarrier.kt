...
): T = DistributedDoubleBarrier(client, barrierPath, memberCount, clientId).use { it.receiver() }
...
constructor(
  ...
  fun enter(
    timeout: Long,
    timeUnit: TimeUnit,
  ): Boolean = enter(timeUnitToDuration(timeout, timeUnit))
  ...
  fun leave(
    timeout: Long,
    timeUnit: TimeUnit,
  ): Boolean = leave(timeUnitToDuration(timeout, timeUnit))
